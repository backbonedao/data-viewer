...
test('applying - one-to-many apply with reordering, local output', async t => {
  ...
  {
    const outputNodes = await linearizedValues(base.view)
    t.same(bufferize(outputNodes.map(v => v.value)), bufferize(['a0-1', 'a0-0', 'b1-1', 'b1-0', 'b0-1', 'b0-0']))
  }
  ...
  {
    const outputNodes = await linearizedValues(base.view)
    t.same(bufferize(outputNodes.map(v => v.value)), bufferize(['b1-1', 'b1-0', 'b0-1', 'b0-0', 'a2-1', 'a2-0', 'a1-1', 'a1-0', 'a0-1', 'a0-0']))
  }
